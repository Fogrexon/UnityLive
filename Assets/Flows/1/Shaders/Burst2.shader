Shader "Custom/VJ/Burst2"
{
  Properties 
  {
    _Face("Face", Float) = 0
    _NowTime("NowTime", Float) = 0
    _Color("Color", Color) = (1,1,1,1)
    _Emission("Emission", Color) = (1,1,1,1)
    _ScaleBox("ScaleBox", Vector) = (1,1,1,1)
    _Scatter("Scatter", Float) = 6.0
  }
  SubShader
  {
    ...
    Pass {
      ...
      {
        return CubicBezier(float2(0.23, 1.0),float2(0.32, 1.0),t);
      }
      ...
      void geom(triangle v2g v[3], inout TriangleStream<g2f> stream)
      {
        ...
        float prog =  easeOutQuint(min(1.0,( _Time.y - _NowTime)/0.7));
        ...
        float4 origin = (rv1 + rv2 + rv3) / 3.0;
        rv1 = lerp(rv1, origin, prog);
        rv2 = lerp(rv2, origin, prog);
        rv3 = lerp(rv3, origin, prog);
